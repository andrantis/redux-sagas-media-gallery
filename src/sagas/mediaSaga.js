...
// and instruction the redux-saga middleware on the next line of action, 
...
export function* searchMediaSaga({ payload }) {
    try {
        ...
        yield [
            put.resolve({ type: types.SHUTTER_VIDEOS_SUCCESS, videos}),
            put({ type: types.SELECTED_VIDEO, video: videos[0] }),
            put.resolve({ type: types.FLICKR_IMAGES_SUCCESS, images}),
